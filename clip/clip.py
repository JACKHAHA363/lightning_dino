...
def tokenize(texts: Union[str, List[str]], context_length: int = 77, with_mask=False):
    """
    ...
    masks = torch.zeros(len(all_tokens), context_length, dtype=torch.long)
    for i, tokens in enumerate(all_tokens):
        if len(tokens) > context_length: # Truncate
            tokens = tokens[:context_length]
        result[i, :len(tokens)] = torch.tensor(tokens)
        masks[i, :len(tokens)] = 1
    if with_mask:
        return result, masks
    else:
        return result
